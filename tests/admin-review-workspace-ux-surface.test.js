...
expectMatch(/function reviewCommunicationStateKind_\(/, "Review communication state labels must normalize to the operator contract");
expectMatch(/function reviewCommunicationActionDescriptor_\(/, "Each review communication action must map to an explicit operator-visible state");
expectMatch(/function renderReviewCommunicationStatePanel_\(/, "Review communication controls must render an explicit state panel");
...
expectMatch(/\.reviewCommStateBadge\.loading\{[\s\S]*background:#fff2dc;[\s\S]*color:#805006;/, "Loading state badge must be explicit and readable");
expectMatch(/\.reviewCommStateBadge\.blocked\{[\s\S]*background:#fdeaea;[\s\S]*color:#8b1f1f;/, "Blocked state badge must be explicit and readable");
expectMatch(/\.reviewCommStateBadge\.disabled\{[\s\S]*background:#e2eaf2;[\s\S]*color:#26384e;/, "Disabled state badge must be explicit and readable");
...
expectMatch(/id="reviewCommStatePanel" class="reviewCommStatePanel" aria-live="polite"/, "Review communication state panel must exist in markup");
expectMatch(/Generate \/ Preview Email[\s\S]*READY|BUSY|LOADING|BLOCKED|DISABLED/, "Review communication state rendering must use explicit contract labels");
expectMatch(/Applicant details still loading\./, "Review communication loading state must be explicit");
expectMatch(/Generate preview first\./, "Review communication send-precondition state must be explicit");
expectMatch(/Open an applicant in Review Workspace first\./, "Review communication no-applicant state must be explicit");

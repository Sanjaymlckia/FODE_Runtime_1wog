...
expectMatch(/function renderDocCardVisual_\(/, "Visible document/status cards must render preview copies when available");
expectMatch(/function isDocCardRenditionCandidate_\(/, "Visible document/status cards must mark configured evidence fields as rendition candidates");
expectMatch(/Birth_ID_Passport_File[\s\S]*Latest_School_Report_File[\s\S]*Transfer_Certificate_File[\s\S]*Passport_Photo_File[\s\S]*Fee_Receipt_File/, "Visible document/status card rendition candidates must cover configured evidence fields");
expectMatch(/canRenderDocCardVisual_\(doc\)[\s\S]*isDocCardRenditionCandidate_\(doc\)/, "Visible document/status card renderer must request server-validated renditions even when MIME is absent from the card DTO");
expectMatch(/class="docVisualPreview"/, "Document/status cards must include a large visual preview area");
expectMatch(/class="documentGalleryZoomBtn docCardZoomBtn"/, "Document/status card previews must support click-to-enlarge");
expectMatch(/Preview copy will be generated\. Original remains available\./, "Unknown MIME visible cards must suppress legacy Download-first wording while server rendition is attempted");
expectMatch(/PNG preview copy available\. Original remains available\./, "Document cards must label generated PNG previews as copies");
expectMatch(/PDF preview copy - first page\. Original remains available\./, "PDF document cards must label first-page preview copies");
expectNoMatch(/detail response raw/, "AdminUI must not dump raw detail DTOs with Drive URLs to browser console");

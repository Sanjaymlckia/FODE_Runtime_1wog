...
assert.match(operatorNextSource, /TEMPORARILY ALLOWED/);
...
assert.match(configSource, /VERSION:\s*"r340"/);
assert.match(configSource, /DEPLOY_VERSION_NUMBER:\s*340/);
assert.match(configSource, /CAPABILITY_GRANTS_SPREADSHEET_CONFIG_KEY:\s*"SPREADSHEET_ID_PROD"/);
assert.equal(runtimeContext.projects.FODE.deployments.adminStaging.expectedRuntime, "r340");
assert.equal(runtimeContext.projects.FODE.deployments.adminStaging.expectedDeploy, 340);

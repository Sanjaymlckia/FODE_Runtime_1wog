...
assert.match(workbench, /var occurredAtPng\s*=\s*occurredAt\s*\?\s*app\.formatPngDate\(occurredAt\)\s*:\s*\(receipt\.occurredAtPng\s*\|\|\s*app\.authorityUnavailable\("receipt timestamp"\)\)[\s\S]*formatPngDate\(preview\.expiresAt\)/, "receipt dates must derive PNG-local display from the immutable technical ISO timestamp; receipt-provided PNG text is fallback only");

...
(async () => {
  ...
  try {
    ...
    assert.equal(cockpitLayout.cardBottoms.length, 3, "backend action packages must render as visible cockpit cards");
    ...
    await page.evaluate(() => { window.EduOpsApp.state.worklistKey = ""; return window.EduOpsApp.requestWorkload({ resetPage: true }); });
    await settled(page);

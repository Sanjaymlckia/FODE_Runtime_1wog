...
function fixtureHtml() {
  ...
  const mock = `<script>
    ...
    window.__populationIntegrity = { schemaVersion: "CANONICAL_POPULATION_INTEGRITY_V1", status: "PASS", authoritySafeToBatch: true, blockCode: "", blockReason: "", populationCount: 253, scannedRowCount: 253, distinctApplicantIdCount: 253, duplicateApplicantIdCount: 0, missingOrInvalidApplicantIdCount: 0, duplicateApplicantIds: [], duplicateRowReferences: [], missingOrInvalidApplicantIds: [], reconciliationFindings: [], evidenceTruncated: false, integrityFingerprint: "CPI-INTEGRATED-SAFE" };
    window.__binding = function (payload) { var query = { product: "FODE", actionabilityState: payload.actionabilityState, worklistKey: payload.worklistKey || "", workScope: payload.workScope, filters: payload.filters || {}, sort: payload.sort || {}, pageSize: payload.pageSize }; return { schemaVersion: "EDUOPS_QUERY_BINDING_V1", authority: "SERVER_AUTHORED", product: "FODE", snapshotId: "SNAP-INTEGRATED", query: query, queryFingerprint: "SERVER::" + JSON.stringify(query), integrityFingerprint: window.__populationIntegrity.integrityFingerprint }; };
...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('label[for="eduopsGlobalSearch"] span').innerText(), "Find any applicant");
    assert.match(await page.locator("#eduopsSearch").getAttribute("placeholder"), /Search within All work types/);
    await page.locator("#eduopsGlobalSearch").fill("Waffi");
    await page.waitForFunction(() => document.querySelector("#eduopsGlobalSearchResults")?.textContent.includes("Keziah Waffi"), null, { timeout: 30000 });
    const globalSearchText = await page.locator("#eduopsGlobalSearchResults").innerText();
    assert.match(globalSearchText, /Lifecycle:\s*Payment pending/);
    assert.match(globalSearchText, /Actionability:\s*Ready for action/);
    assert.match(globalSearchText, /Work package:\s*Payment follow-up/);
    assert.match(globalSearchText, /Next action:\s*Payment follow-up/);
    assert.match(globalSearchText, /Open correct work package/);
    const searchPayload = await page.evaluate(() => window.__rpcCalls.filter((call) => call.name === "eduops_searchApplicants").pop().payload);
    assert.deepEqual(Object.keys(searchPayload).sort(), ["expectedSnapshotId", "limit", "product", "query"].sort(), "global applicant search must not carry active package filters");
    await page.locator('[data-search-worklist="FODE-26-002959"]').click();
    await settled(page);
    assert.equal(await page.locator("#eduopsGlobalSearch").inputValue(), "Waffi", "global search context remains available after correct-package handoff");
    assert.equal(await page.evaluate(() => window.EduOpsApp.state.worklistKey), "PAYMENT_FOLLOW_UP", "global search handoff opens the backend-projected work package");
    assert.equal(await page.evaluate(() => window.EduOpsApp.state.actionabilityState), "READY", "global search handoff opens the backend-projected actionability state");
    assert.equal(await page.locator("#eduopsWorklistRows [data-applicant-row]").getAttribute("data-applicant-row"), "FODE-26-002959");

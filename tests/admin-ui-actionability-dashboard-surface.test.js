...
assert.match(functionSource("actionabilityEligibleNowCountForGroup_"), /key === "APPLICANT"[\s\S]*workload\.READY[\s\S]*filter\(actionabilityIsSelectable_\)/, "Applicant Action eligible-now count must reconcile with server workloadSummary READY and fallback to selectable DTO rows");
assert.match(functionSource("actionabilityCoolingOffCountForGroup_"), /key === "APPLICANT"[\s\S]*workload\.COOLING_OFF[\s\S]*COOLING_OFF/, "Applicant Action cooling-off count must reconcile with server workloadSummary COOLING_OFF and fallback to row DTO state");

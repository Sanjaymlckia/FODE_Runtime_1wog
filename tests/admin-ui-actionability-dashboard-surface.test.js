...
assert.match(adminUi, /id="batchCommModalBack"/, "Batch communication must provide a dedicated modal surface");
assert.match(adminUi, /id="batchCommModalBack" class="modalBack"/, "Batch modal container must be renderable as a real modal");
assert.match(functionSource("openBatchCommunicationModal_"), /back\.classList\.add\("open"\)/, "Batch Communication must visibly open the modal container");
assert.match(adminUi, /id="batchCommTemplateGallery"/, "Batch modal must expose the shared template gallery pattern");
...
assert.match(adminUi, /id="batchCommRecipients"/, "Batch modal must expose recipient preview and quick exclusion");
assert.match(adminUi, /Recipient count/, "Batch modal preview must show recipient count");
assert.match(adminUi, /id="btnBatchCommPreview"[\s\S]*>Preview<\/button>[\s\S]*id="btnBatchCommSend"[\s\S]*>Send Batch<\/button>/, "Batch modal must expose preview and confirm/send controls");

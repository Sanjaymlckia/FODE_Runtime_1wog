...
assert.match(adminUi, /id="btnStandaloneBatchCommPreview"[\s\S]*>Generate Preview<\/button>[\s\S]*id="btnStandaloneBatchCommSend"[^>]*disabled>Bulk send prohibited<\/button>/, "Batch modal must preserve preview and disable bulk send");
assert.match(adminUi, /id="standaloneBatchCommConfirm"/, "Batch modal must render an in-app confirmation panel");
assert.match(adminUi, /batchCommWorkflow[\s\S]*Template[\s\S]*Preview[\s\S]*Recipients[\s\S]*Confirm[\s\S]*Bulk disabled/, "Batch modal must show the prohibited bulk workflow");
...
assert.match(functionSource("renderBatchCommunicationModal_"), /Readiness[\s\S]*batchCommSendProhibitedReason_[\s\S]*Communication Authority/, "Batch modal readiness must display both the prohibition and Communication Authority source");
...
assert.match(functionSource("sendBatchCommunicationModal_"), /BATCH_SEND_PROHIBITED/, "Batch modal send attempt must return the local prohibition result");
assert.doesNotMatch(functionSource("sendBatchCommunicationModal_"), /admin_sendStageBatch|admin_sendSelectedApplicantBatch/, "Batch modal must not retain client bulk send RPCs");
...
assert.match(adminUi, /Preview diagnostics are available\. Bulk sending remains prohibited/, "Batch UI must explain the current prohibition");
...
assert.match(functionSource("batchCommCanSend_"), /return false/, "Batch send must remain disabled regardless of preview state");
...
assert.match(functionSource("batchCommConfirmHtml_"), /Bulk Send Prohibited[\s\S]*batchCommSendProhibitedReason_\(\)/, "Batch confirmation panel must explain the prohibition");
assert.doesNotMatch(functionSource("batchCommConfirmHtml_"), /sendBatchCommunicationModal_/, "Batch confirmation panel must not render an executable send control");
...
assert.match(adminUi, /class="btn actionabilityReviewBtn operatorControl operatorControlPrimary"[\s\S]*>Open Review<\/button>/, "Row Review must use explicit primary operator control semantics");
...
assert.match(adminJs, /coolingOffUntil:\s*clean_\(row\.coolingOffUntil \|\| ""\)/, "Hidden record DTO must pass through existing cooling-off next-action date");
assert.match(adminJs, /selectBlockReason:\s*clean_\(row\.selectBlockReason \|\| ""\)/, "Hidden record DTO must pass through existing block reason");
assert.match(adminJs, /worklistKey:\s*clean_\(row\.worklistKey \|\| ""\)/, "Hidden record DTO must pass through existing route/worklist authority");
...
assert.doesNotMatch(cssRule(".actionabilityMiniChip"), /font-size:10px/, "Operational chips must not use 10px diagnostic text");
assert.match(cssRule(".actionabilityReviewBtn"), /min-width:126px;[\s\S]*font-size:14px;/, "Open Review controls must have enough width and readable text");
...
assert.match(adminUi, /Applicant ID ' \+ esc\(row\.applicantId \|\| "unavailable"\)/, "Hidden drill-down must expose bounded applicant identity fallback");

...
assert.match(selectedBatchSend, /bulkCommunicationProhibitionResult_/, "Selected cohort send must route through the shared bulk prohibition authority");
...
assert.match(selectedBatchSend, /selectedApplicantPopulationIntegrityGate_[\s\S]*bulkCommunicationProhibitionResult_/, "Selected cohort send must prove population integrity before the global bulk prohibition");
assert.doesNotMatch(selectedBatchSend, /sendApplicantMessage_\(/, "Selected cohort send must not retain a recipient send loop while Batch is prohibited");
assert.doesNotMatch(selectedBatchSend, /readSelectedApplicantBatchPreviewCache_\(/, "Selected cohort send must not consume preview cache after the terminal bulk prohibition");
...
assert.match(stageSend, /stageBatchPopulationIntegrityGate_[\s\S]*bulkCommunicationProhibitionResult_/, "Stage Batch send must prove population integrity before the global bulk prohibition");
...
assert.doesNotMatch(stageSend, /sendApplicantMessage_\(/, "Stage Batch send must not retain a recipient send loop while Batch is prohibited");
assert.doesNotMatch(stageSend, /readStageBatchPreviewCache_\(adminEmail\)/, "Stage Batch send must not consume preview cache after the terminal bulk prohibition");

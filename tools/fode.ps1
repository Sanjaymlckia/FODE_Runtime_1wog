...
if ($Command -eq 'close') {
  ...
  if ($closeLease) { $args += @('-OwnerLease', $closeLease) }
  if ($ClearPendingAcceptance) { $args += '-ClearPendingAcceptance' }
  if ($AcceptBaselineAdvance) { $args += '-AcceptBaselineAdvance' }
  $closeOutput = @(& $SessionTool @args 2>&1)

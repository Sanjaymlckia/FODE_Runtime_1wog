...
const captureUrl = process.env.OPSEDU_CAPTURE_URL || process.argv[2] || defaultAdminUrl;
const expectedRuntime = process.env.OPSEDU_EXPECTED_RUNTIME || "r365";
const expectedDeploy = Number(process.env.OPSEDU_EXPECTED_DEPLOY || 365);
...
async function capture() {
  ...
  if (String(url.searchParams.get("view") || "").toLowerCase() !== "eduops") {
    fail("OpsEdu preview capture must target the accepted Admin ?view=eduops surface.");
  }

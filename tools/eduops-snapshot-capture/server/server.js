...
function renderEduOpsPreviewHtml() {
  ...
  html = html.replace('<?!= HtmlService.createHtmlOutputFromFile("EduOps_OperationsWorkspaceStyles").getContent(); ?>', readText(path.join(repoRoot, "EduOps_OperationsWorkspaceStyles.html")));
  html = html.replace('<?!= HtmlService.createHtmlOutputFromFile("EduOps_ClientOperationsWorkspace").getContent(); ?>', readText(path.join(repoRoot, "EduOps_ClientOperationsWorkspace.html")));

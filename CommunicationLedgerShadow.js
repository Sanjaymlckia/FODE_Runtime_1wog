...
function fodeLedgerPrepareIndividual_(identity, applicantId, payload, authorityContext, options) {
  ...
  if (result.status === "REPLAY" && response.status === "SENT") return { ok: true, replay: true, idempotent: true, finalized: true, status: "SENT", response: response, commandId: response.commandId || request.commandId, eventId: response.eventId || "", communicationId: response.communicationId || request.payload.communicationId, operationId: request.operationId, previewId: request.previewId, receiptId: request.receiptId, idempotencyKey: request.idempotencyKey, ledgerEnvironment: ledgerEnvironment };
...
function fodeLedgerFinalizeIndividual_(identity, applicantId, payload, authorityContext, legacyResult, options) {
  ...
  return { ok: true, status: response.status || finalState, replay: result.status === "REPLAY", idempotent: result.status === "REPLAY", response: response, commandId: request.commandId, eventId: response.eventId || "", communicationId: response.communicationId, operationId: request.operationId, previewId: request.previewId, receiptId: request.receiptId, idempotencyKey: request.idempotencyKey };
}
